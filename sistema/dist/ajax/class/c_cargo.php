...
class Tematica
{
    ...
    {
        $dataTotal = Conexion::buscarRegistro("SELECT count(*) total FROM cargo where in estado in ('A','I') ");
    ...
    function actualizar($cod_usuario, $cod_cargo, $nombre)
    {
        if ($cod_usuario == "") {
            $respuesta['status'] = "informacion";
            $respuesta['mensaje'] = "Por favor vuelve a iniciar sesion";
        } else {
            $consulta = "UPDATE cargo set nombre = '$nombre', fecha_actualizado = now(), usuario_actualizado = $cod_usuario where cod_cargo = $cod_cargo ";
            $query = Conexion::UpdateRegistro($consulta);
            if ($query) {
                $respuesta['status'] = "correcto";
    ...
    function cambiarEstado($cod_usuario, $cod_cargo, $estado)
    {
        if ($cod_usuario == "") {
            $respuesta['status'] = "informacion";
            $respuesta['mensaje'] = "Por favor vuelve a iniciar sesion";
            ...
            $consulta = "UPDATE cargo set estado = '$estado', fecha_actualizado = now(), usuario_actualizado = $cod_usuario where cod_cargo = $cod_cargo ";
            $query = Conexion::UpdateRegistro($consulta);
            if ($query) {
                $respuesta['status'] = "correcto";
    ...
    function eliminar($cod_usuario, $cod_cargo)
    {
        if ($cod_usuario == "") {
            $respuesta['status'] = "informacion";
            $respuesta['mensaje'] = "Por favor vuelve a iniciar sesion";
        } else {
            $consulta = "UPDATE cargo set estado = 'D', fecha_eliminado = now(), usuario_eliminado = $cod_usuario where cod_cargo = $cod_cargo ";
            $query = Conexion::UpdateRegistro($consulta);
            if ($query) {
                $respuesta['status'] = "correcto";
